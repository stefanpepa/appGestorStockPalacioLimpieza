...
function importarJSON(archivo) {
  ...
  reader.onload = function(e) {
    try {
      ...
      if (Array.isArray(data.items) && Array.isArray(data.ventas)) {
        items = data.items;
        ventas = data.ventas;
        save();
        render();
...
if (toolbar) {
  ...
  btnExportar.className = "btn secondary";
  ...
  inputImportar.accept = ".json,application/json";
  ...
  btnImportar.style.marginLeft = "8px";
...
// --- Placeholder dinámico para "objeto" ---
const ejemplosObjeto = ["Lavandina", "Jabón", "Detergente", "Desinfectante", "Shampoo", "Suavizante", "Alcohol", "Limpiavidrios"];
let indexObjeto = 0;
const inputObjeto = document.getElementById("objeto");

setInterval(() => {
  // Solo cambiar si el input está vacío y no enfocado
  if (document.activeElement !== inputObjeto && inputObjeto.value.trim() === "") {
    inputObjeto.placeholder = `Ej: ${ejemplosObjeto[indexObjeto]}`;
